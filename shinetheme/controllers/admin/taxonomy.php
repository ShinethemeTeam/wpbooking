...
if(!class_exists('Traveler_Admin_Taxonomy_Controller'))
{
	class Traveler_Admin_Taxonomy_Controller extends Traveler_Controller
	{
		...
		function _traveler_create_taxonomy()
		{
			...
			// Error checking
			if (!$taxonomy_label || !$taxonomy_slug) {
				$error = __('Please, provide an taxonomy name .', 'traveler-booking');
			} elseif (strlen($taxonomy_slug) >= 28) {
				$error = sprintf(__('Slug “%s” is too long (28 characters max). Shorten it, please.', 'traveler-booking'), ($taxonomy_slug));
			} elseif (in_array($taxonomy_slug, $reserved_terms)) {
				$error = sprintf(__('Slug “%s” is not allowed because it is a reserved term. Change it, please.', 'traveler-booking'), ($taxonomy_slug));
			} else {
