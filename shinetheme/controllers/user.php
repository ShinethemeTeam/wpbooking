<?php
    if ( !class_exists( 'WPBooking_User' ) ) {
        class WPBooking_User
        {
            ...
            {
                add_action( 'init', [ $this, '_add_shortcode' ] );
                /**
                 ...
                 * @since  1.0
                 */
                add_action( 'init', [ $this, '_login_register_handler' ], 11 );
                 ...
                 * @author dungdt
                 */
                add_action( 'wpbooking_register_success', [ $this, '_send_registration_email' ], 10, 1 );
                 ...
                 * @author dungdt
                 */
                add_filter( 'wpbooking_registration_email_shortcode', [ $this, '_get_shortcode_content' ], 10, 3 );
                 ...
                 * @author dungdt
                 */
                add_action( 'wp_ajax_wpbooking_register_email_preview', [ $this, '_preview_email' ] );
                 ...
                 * @author dungdt
                 */
                add_action( 'init', [ $this, '_myaccount_page_handler' ], 20 );
                 ...
                 *
                 */
                add_filter( "pre_get_avatar", [ $this, '_change_profile_avatar' ], 10, 3 );
                 ...
                 * @author tienhd
                 */
                add_action( 'login_form_lostpassword', [ $this, '_retrieve_password' ] );
                 ...
                 * @author tienhd
                 */
                add_filter( 'logout_url', [ $this, '_redirect_logout_url' ], 10, 2 );
...
//            add_filter('login_url',array($this,'_redirect_login_url'),10,3);
                 ...
                 * @author tienhd
                 */
                add_action( 'login_form_rp', [ $this, '_redirect_reset_password_url' ] );
                add_action( 'login_form_resetpass', [ $this, '_redirect_reset_password_url' ] );
                 ...
                 * @author tienhd
                 */
                add_action( 'login_form_resetpass', [ $this, '_reset_password' ] );
                 ...
                 * @author tienhd
                 */
                add_filter( 'retrieve_password_message', [ $this, '_email_retrieve_password_template' ], 10, 4 );
                 ...
                 * @author tienhd
                 */
                add_action( 'send_password_change_email', [ $this, '_send_email_changed_password' ], 10, 3 );

                add_action( 'template_redirect', [ $this, '_redirect_register_url' ] );
            ...
            function validate_service()
            {
                ...
                if ( !$service or $service->post_type != 'wpbooking_service' ) {
                    ...
                    wp_redirect( add_query_arg( [ 'tab' => 'services' ], $myaccount_page ) );
                    die;
                }
            ...
            function get_tabs()
            {
                ...
                $tabs[ 'booking_history' ] = [
                    'label' => esc_html__( 'Your Booking', 'wp-booking-management-system' ),
                ];
                $tabs[ 'edit_profile' ]    = [ 'label' => esc_html__( 'Edit Profile', 'wp-booking-management-system' ) ];

                $tabs[ 'logout' ] = [ 'label' => esc_html__( 'Logout', 'wp-booking-management-system' ) ];

                return apply_filters( 'wpbooking_myaccount_tabs', $tabs );
            }

            function _myaccount_shortcode( $attr = [], $content = false )
            {
            ...
            {
                add_shortcode( 'wpbooking-myaccount', [ $this, '_myaccount_shortcode' ] );
            }
             ...
             */
            function order_create_user( $data = [], $meta_fields = [] )
            {
                ...
                $user_name = $this->generate_username( $data[ 'user_email' ], $data[ 'first_name' ], $data[ 'last_name' ] );
                if ( $user_name ) {
                    ...
                    if ( !is_wp_error( $create_user ) ) {
                        ...
                        if ( !empty( $meta_fields ) and is_array( $meta_fields ) ) {
                            $f = [ 'user_email', 'user_first_name', 'user_last_name', 'user_phone', 'user_address', 'user_postcode', 'user_apt_unit' ];
            ...
            {
                if ( !empty( $meta_fields ) and is_array( $meta_fields ) ) {
                    $f = [ 'user_email', 'user_first_name', 'user_last_name', 'user_phone', 'user_address', 'user_postcode', 'user_apt_unit', 'passengers' ];
                    foreach ( $meta_fields as $key => $value ) {
                        if ( array_key_exists( $key, $f ) )
             ...
             * @return string
             */
            function generate_username( $email, $firstname, $lastname )
            {
                ...
                if ( username_exists( $user_name ) ) return $this->generate_username( $email, $firstname, $lastname );

                return apply_filters( 'wpbooking_generated_username', $user_name, $email, $firstname, $lastname );
            }
            ...
            public function _email_retrieve_password_template( $message, $key, $user_login, $user_data )
            {
                ...
                $message = wpbooking_load_view( 'emails/templates/lost_password', [ 'key' => $key, 'user_login' => $user_login ] );
                ...
                if ( $is_success ) {
                    if ( !empty( $user ) ) {
                        ...
                        if ( is_email( $to ) ) {
                            ...
                            $message = wpbooking_load_view( 'emails/templates/changed_password', [ 'user' => $user ] );
