...
if( is_array( $data['value'] ) && !empty( $data['value'] ) ){
	...
	foreach( $data['value'] as $key => $value ){
		...
		$field .= '<div style="margin-bottom: 7px;"><label for="'.esc_html( $data['id'] ). '_' .esc_html( $key ) . '"><input type="checkbox" name="'. esc_html( $data['id'] ) .'[]" id="'. esc_html( $data['id'] ). '_'. esc_html( $key ) . '" class="'. esc_html( $data['class'] ) . '" value="'. esc_html( $key ) .'" ' . $checked .'> <span>'. esc_html( $value ) .'</span></label></div>';
	}
}
